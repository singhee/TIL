...
아래의 코드와 위에서 Promise로 만든 코드를 비교해 보도록 해보자. 둘은 같은 동작을 하는 코드이다. 하지만, 둘을 비교했을 때 어떤 코드가 이해하기 더 쉬울까? 당연히 아래의 코드가 훨씬 직관적이고 알아보기 쉽다. Promise 코드에서 아래와 같이 할 수 없는 이유는 처음에도 언급했듯이 자바스크립트가 싱글-스레드 기반의 언어이기 때문이다. 또한, 각각의 네트워크 요청이 값을 반환하기 전까지 프로그램 전체가 멈춰서 대기를 해야한다면 이 프로그램은 너무 느려서 사용할 수가 없을 것이다. 
...
```
이 코드가 우리가 원하는 대로 실행이 된다면, 정말 좋을 것이다. 하지만, 우리가 원하는 대로 되지 않을 것이다. 일단, `yield`를 통해 실행을 멈추고 제어권을 넘겨준 것 까지는 좋았다. 하지만, `getId()`가 작업을 완료하는 순간 다시 반환값과 함께 제어권을 가져오려면 이터레이터의 `next()`함수를 호출해 주어야 할 것이다. 현재로서는 작업 완료 시점을 알고 있는 `getId()`내부에서 직접 호출하는 방법밖에 없는데, 그렇게 된다면 이터레이터와의 강한결합이 형성된다. 즉 위의 코드는 다음과 같이 데이터를 반환하는 모든 함수의 마지막에 `next()`를 호출하는 코드가 추가되어야 할 것이다.

```javascript
const iterator = orderCoffee('010-2581-7599');
iterator.next();
...
```
이래서는 제너레이터가 콜백보다 더 나빠진 경우가 될 것이다. 

## 제너레이터와 프로미스의 만남
만약 모든 함수가 프라미스를 반환한다면 각각의 함수가 제어권을 직접 다루지 않고, 제3자게 위임할 수 있지 않을까? 우선 프로미스의 예제에서처럼 모든 `getXXX`함수는 프로미스를 반환한다고 가정하자. 이제 누군가가 이터레이터를 생성해서 함수가 끝날때까지 반복해서 실행시켜 주면 된다.
...
이터레이터를 생성해서 `next()`를 실행하면 결과의 `value`값으로 프로미스를 반환하고, 프로미스의 `then()`메서드에서 다시 이터레이터의 `next()` 를 실행한다. 이런 식으로 이터레이터가 `done:true`를 반환할 때까지 순환하면서 호출해준다. 즉, `next()` -> `yield` -> `then()` -> `next()` 의 순환흐름에 따라 실행되는 것이다. 

이렇게 프로미스와 제너레이터를 함께 사용하면 각각의 함수에서 제너레이터를 신경 쓰지 않고도 외부에서 제어할 수 있다. 이것이 바로 제너레이터를 활용해 비동기인 코드를 마치 동기식 코드인 것처럼 작성한 코드인 것이다. 그리고 이미 이러한 기능을 구현해 놓은 아주 좋은 라이브러리들이 있다. 그중 가장 널리 쓰이는 라이브러리는 [co](https://github.com/tj/co)이다. co는 제너레이터를 쉽게 사용할 수 있는 아주 편리한 두 가지 함수를 제공한다. 시간이 된다면 co를 살펴보고 활용해 보도록 하자. 
